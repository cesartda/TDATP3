...
def main(file_stations,file_railways,people_per_wagon,min_wagon_per_line,num_wagons_in_system):
    ...
    necessary_num_wagons = 0
    for i in lines_railways_graph[1][1]:
        necessary_num_wagons += i[1]
    if necessary_num_wagons > num_wagons_in_system:
        print("No es factible debido a la cantidad de vagones")
        exit()
    print("Es factible segun este modelo. Se necesitarian "+str(necessary_num_wagons) + " vagones.")
...
def create_lines_railways_graph(subway_people_graph,station_dict,subway_people_graph_residual,people_per_wagon,min_wagon_per_line):
    ...
    # para arriba
    # Para encontrar los tramos se hace un BFS, y no se pasa entre líneas
    # (suponemos que las combinaciones admiten cantidad de gente infinita,
    # ya que no necesitan vagones)
    ...
    # Creo la primer línea. lines_nodes_pos tiene las posiciones de las líneas
    this_line = 2
...
def create_subway_graph(file_stations,file_railways,max_people_per_wagon):
    ...
    # entonces grafo[0] es una lista con: nombre, lista de aristas incidentes
    # grafo[0][1] es la lista de aristas que inciden en el nodo
    # Una arista es una lista con la posición del nodo al que incide y la capacidad
